...
class BasPreparedReport(Document):
		...
		try:
			...
			if report_doc.prepared_report:
				...
				try:
					...
					frappe.log_error("Bas Prepared Report: make_prepared_report failed", str(e))

			else:
				# Standard Report (Run Immediately)
				try:
					result = run(report_name, filters)
					frappe.log_error("Bas Prepared Report: run failed",result)
					...
					self.status = "Error"
					self.error_message = str(e)
					frappe.log_error("Bas Prepared Report: run failed", str(e))

		except Exception as outer_e:
			self.status = "Error"
			self.error_message = str(outer_e)
			frappe.log_error("Bas Prepared Report: general failure",str(outer_e))
