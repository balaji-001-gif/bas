...
@frappe.whitelist()
def update_bas_reports(doc,event):
    # frappe.log_error("status update_bas_reports doc",doc)
    try:
        
        if frappe.db.exists("Bas Prepared Report", {"prepared_report": doc.name}):
            ...
            if reports:
                report = frappe.get_doc("Bas Prepared Report", reports[0]["name"])
                ...
                if files:
                    ...
                    report.save()
                    frappe.db.commit()
                    frappe.log_error("file_content update_bas_reports doc",file_content)
                else:
                    report.status = doc.status
                    report.error = doc.error_message or ""
                    report.finished_at = doc.report_end_time
                ...
                frappe.log_error("No Bas Prepared Report found for the given prepared_report", doc.name)
        else:
            frappe.log_error("No Bas Prepared Report", doc.name)

    except Exception as e:
        frappe.log_error("update_bas_reports error", str(e))
...
def login_user(usr, pwd):
    ...
    if user_exist > 0:
        ...
        employee_data = frappe.db.get_all('Bas Employee', filters={'user_id': user_email}, fields=['*'])
        if employee_data :
            settings = frappe.get_doc('Bas Settings')
...
def verifyOTP(usr, pwd):
    ...
    if user_exist > 0:
        ...
        if employee_data :
            ...
            frappe.log_error("bas_api.py login_user employee_data", {
                "status": True,
...
def storelocation():
    try:
        ...
        for loc in locations:
            ...
            if frappe.db.exists("Bas Employee", {"user_id": user}):
                employee = frappe.get_doc("Bas Employee", {"user_id": user})
...
def leave_balance():
    try:
        employee = frappe.get_doc("Bas Employee", {"user_id": frappe.session.user})
        if employee:
            frappe.response.message = {
                'status': True,
...
def user_details():
    try:
        ...
        if user:
            employee_data = frappe.db.get_all('Bas Employee', filters={'user_id': frappe.session.user}, fields=['*'])
            if employee_data :
                settings = frappe.get_doc('Bas Settings')
...
@frappe.whitelist()
def create_bas_post(title: str, content: str):
    doc = frappe.get_doc({"doctype": "Bas Post", "title": title, "content": content})
...
@frappe.whitelist()
def get_bas_posts(limit_start: int = 0, limit_page_length: int = 10):
    try:
